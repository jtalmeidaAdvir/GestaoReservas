...
const ExcelImportPage = () => {
  ...
  const handleImportReservations = async () => {
    ...
    for (const reservation of reservations) {
      ...
      try {
        const lastReservationResponse = await fetch(`http://94.143.231.141:3010/reservations/last`);
      ...
      try {
        const checkResponse = await fetch(`http://94.143.231.141:3010/reservations/by-reserva/${updatedRow.reserva}`);
        if (checkResponse.ok) {
          const existingReservation = await checkResponse.json();
          const response = await fetch(`http://94.143.231.141:3010/reservations/${existingReservation.id}`, {
            method: "PUT",
          ...
          const response = await fetch(`http://94.143.231.141:3010/reservations/create`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
