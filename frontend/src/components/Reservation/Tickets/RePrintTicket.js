...
const handleRePrintTicket = (row, datatrip, formatDate, returnPDF = false) => {
  ...
  const ticketCount = 0; // Só um bilhete
  ...
  if (finalPrice > 0) {
    ...
    pdf.text("----", contentAreaX + usableWidth / 2 + fieldMargin + 5, yOffset + 30);
  }
  
  const dataIdaTexto = datatrip ? formatDatePorExtenso(datatrip) : "----";
...
export default handleRePrintTicket;
