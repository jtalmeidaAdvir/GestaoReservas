...
const socket = io("http://192.168.1.10:3000"); // Conectar ao WebSocket
...
const Reservation = ({tripId}) => {
    ...
    const handleRowEdit = async (updatedRow, oldRow = {}) => {
        try {
            ...
            if (!updatedRow.reserva) {
                let lastReservationResponse = await fetch(`http://192.168.1.10:3000/reservations/last`);
            ...
            let checkResponse = await fetch(`http://192.168.1.10:3000/reservations/by-reserva/${updatedRow.reserva}`);
            if (checkResponse.ok) {
                const existingReservation = await checkResponse.json();
                let response = await fetch(`http://192.168.1.10:3000/reservations/${existingReservation.id}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                ...
                let response = await fetch(`http://192.168.1.10:3000/reservations/create`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
            ...
            if (oldRow && oldRow.volta !== undefined && oldRow.volta !== updatedRow.volta && updatedRow.volta) {
                ...
                const tripRegressoResponse = await fetch(
                    `http://192.168.1.10:3000/trips/return?origem=${destinoDeIda}&destino=${origemDeIda}&dataviagem=${dbFormatDate}`
                );
    ...
    const fetchReservations = async () => {
        ...
        try {
            const response = await fetch(`http://192.168.1.10:3000/trips/trip/${tripId}`);
            ...
            if (data.trip) {
                ...
                const citiesResponse = await fetch(`http://192.168.1.10:3000/cities`);
    ...
    onClick={async () => {
        const response = await fetch(`http://192.168.1.10:3000/trips`);
...
<SelectReturnSeatModal
    ...
    onConfirm={async (selectedSeat) => {
        ...
        if (returnReservationData) {
            ...
            let resCreateReturn = await fetch(`http://192.168.1.10:3000/reservations/create`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
